...
public class AddObstacleView extends JPanel implements IView {
	...
	private void enableAddingFunction(boolean enable) {
		// added this instead of copying all of them // LB
		tbxWidth.setEnabled(enable);
		tbxHeight.setEnabled(enable);
		tbxXPosition.setEnabled(enable);
		tbxYPosition.setEnabled(enable);
		rdbtnTypeTriangle.setEnabled(enable);
		rdbtnTypeRectangle.setEnabled(enable);
		rdbtnTypeCircle.setEnabled(enable);
		btnAddObstacle.setEnabled(enable);
	}
